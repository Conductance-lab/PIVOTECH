...
double param_b_f = 600000.00;
double param_c_f = 600000.00;
    ...
    auto update_channel = [&](bool is_open, uint gpio_pin, const PwmCalcResult &cfg, uint8_t output_mode)
    {
        if (is_open)
        {
            ...
            if (gpio_pin == TDPWMOUT)
            {
                ...
                // pwm_set_gpio_level(LEDTDPIN, cfg.wrap - cfg.level); // LEDTDPIN 输出与 TDPWMOUT 反相的占空比，以便区分显示（占空比越大 LED 越暗）
                // if (output_mode == 2)
                // {
                //     // DC 100输出
...
void Update_pwmconfig()
{
    ...
    pwm_results[idx] = calc_pwm_params(clock_freq, *PWMitem[idx].frec->param, *PWMitem[idx].duty->param, PWMitem[idx].cfg.output_mode);
    if (PWMitem[idx].open)
    {
        Update_PwmOutput();
    }
